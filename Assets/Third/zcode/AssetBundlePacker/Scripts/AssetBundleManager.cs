...
namespace zcode.AssetBundlePacker
{
    ...
    public class AssetBundleManager : MonoSingleton<AssetBundleManager>
    {
        ...
        ///   加载一个资源
        /// </summary>
        public T LoadAsset<T>(string asset, bool unload_assetbundle = true, bool unload_all_loaded_objects = false) where T : Object
        {
            try
            {
                if (!IsReady)
        ...
        ///   异步加载一个资源
        /// </summary>
        public AssetBundleRequest LoadAssetAsync<T>(string asset, bool unload_assetbundle = true, bool unload_all_loaded_objects = false) where T : Object
        {
            try
            {
                if (!IsReady)
        ...
        public bool LoadScene(string scene_name, LoadSceneMode mode = LoadSceneMode.Single, bool unload_assetbundle = true, bool unload_all_loaded_objects = false)
        {
            try
            {
                if (!IsReady)
        ...
        ///   异步加载场景
        /// </summary>
        public AsyncOperation LoadSceneAsync(string scene_name, LoadSceneMode mode = LoadSceneMode.Single, bool unload_assetbundle = true, bool unload_all_loaded_objects = false)
        {
            try
            {
                if (!IsReady)
        ...
        IEnumerator Preprocess()
        {
            ...
            else
            {
                ...
                if (initial == null)
                {
                    Error(emErrorCode.PreprocessError , "Initial path don't contains " + Common.RESOURCES_MANIFEST_FILE_NAME + "!");
                    yield break;
                } 
        ...
        bool PreprocessFinished()
        {
            ...
            if (ResourcesManifest == null)
            {
                Error(emErrorCode.LoadResourcesManiFestFailed, "Can't load ResourcesInfo file!");
                return false;
            }
