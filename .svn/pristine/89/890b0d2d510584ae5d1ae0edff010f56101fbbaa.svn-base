...
{
    static private int GetAlignSize(Type t)
    {
        int align = 1;
        ...
        {
            //windows默认按8字节对齐
            if (8 > t.StructLayoutAttribute.Pack)
            {
                align = t.StructLayoutAttribute.Pack;
                return align;
            }
    ...
    static private int ToByte(Type t, object o, MemoryStream ms, int offset, ref int alignment, ref int align)
    {
        int size = 0; //local size
        int pack = 0; 
#if UNITY_EDITOR || UNITY_STANDALONE_WIN
        pack = (0 == t.StructLayoutAttribute.Pack) ? 8 : t.StructLayoutAttribute.Pack;
#else
        pack  = (0 == t.StructLayoutAttribute.Pack) ? 4 : t.StructLayoutAttribute.Pack;
#endif
        ...
        try
        {
            foreach (FieldInfo fi in t.GetFields())
            {
                ...
                if ((ft.IsArray) || (ft.Equals(typeof(String))))
                {
                    ...
                    OdaoFieldAttribute attr = (OdaoFieldAttribute)objs[0];
                    ...
                    else if (ft.IsClass)
                    {
                        ...
                        foreach (object ob in v)
                        {
                            ...
                            int size1 = ToByte(ob.GetType(), ob, stream, 0, ref alignment, ref localalign);
                            if (align < localalign)
                            {
                                align = localalign;
                            }
                ...
                else if (ft.IsClass)
                {
                    ...
                    MemoryStream stream = new MemoryStream();
                    int size1 = ToByte(ft, v, stream, 0, ref alignment, ref localalign);
                    if (align < localalign)
                    {
                        align = localalign;
                    }
    ...
    static private byte[] ToByte(object o, int offset)
    {
        MemoryStream ms = new MemoryStream();
        int alignment = 1; //max size of object alignment
        int align = 1; //max size of local alignment

        int size = ToByte(o.GetType(), o, ms, offset, ref alignment, ref align);
    ...
    static private void ToObject(Type t, object o, byte[] buffer, ref int offset, ref int alignment)
    {
        int align = 1; //local alignment
        int pack = 4; //pack size of object o
...
//#endif
        try
        {
            foreach (FieldInfo fi in t.GetFields())
            {
                ...
                if ((ft.IsArray) || (ft.Equals(typeof(String))))
                {
                    ...
                    if (ft.Equals(typeof(String)))
                    {
                        //modified by hud on 2016-02-01
                        fi.SetValue(o, System.Text.Encoding.UTF8.GetString(buffer, offset, attr.SizeConst));
                       // Debug.Log("当前编码格式为...................." + System.Text.Encoding.Default.WebName);
    ...
    static private T ToObject<T>(byte[] buffer, ref int offset)
    {
        Type t = typeof(T);
        object o = Activator.CreateInstance(t);

        int alignment = 1;//一律根据服务器对齐方式，按4个字节对齐
                          //int alignment = 4;
                          //#if UNITY_EDITOR || UNITY_STANDALONE_WIN
                          //     alignment   = (0 == t.StructLayoutAttribute.Pack) ? 4 : t.StructLayoutAttribute.Pack;
                          //#else
                          //        alignment   = (0 == t.StructLayoutAttribute.Pack) ? 4 : t.StructLayoutAttribute.Pack;
                          //#endif

        ToObject(t, o, buffer, ref offset, ref alignment);
        //UnityEngine.Debug.Log("alignment:" + alignment + "  offset:" + offset);
    ...
    {
        int offset = 0;
		return XConvert.ToObject<T>(bytes, ref offset);
    }

	public static T ConvertToObject<T>(byte[] bytes, ref int offset)
    {
		return XConvert.ToObject<T>(bytes, ref offset);
    }

    public static byte[] ConvertToByte(object o, int offset = 8)
    {
        return ToByte(o, offset);
    }
}
