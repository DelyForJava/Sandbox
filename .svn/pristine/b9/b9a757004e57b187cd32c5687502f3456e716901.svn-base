...
namespace AesLib
{
    public class AESCrypt
    {
        ...
        /////////////////////////////////// decrypt ////////////////////////////////////////
        private static aes_ctx DecryptKey(string strKey)
        {
            aes_ctx ctx = null;
        ...
        private static byte[] DecryptBuf(byte[] data, aes_ctx ctx)
        {
        ...
        private static void DecryptBlk(byte[] in_blk, byte[] out_blk, aes_ctx ctx)
        {
            uint[] b0 = new uint[4];
            uint[] b1 = new uint[4];
            int nc = BLOCK_SIZE >> 2;
        ...
        private static void inv_rnd(uint[] y, uint[] x, uint[] k, int index, int c, int nc, uint[][] tab)
        {
            y[c] = k[c + index] ^
        ...
        private static uint inv_var(uint[] x, int r, int c, int nc)
        {
            return
                r == 0 ?
                    (c == 0 ? x[0]
        ...
        private static void kdf4(uint[] k, int i, uint[] ss, uint[][] tab)
        {
        ...
        private static void kd4(uint[] k, int i, uint[] ss, uint[][] tab)
        {
            ss[4] =
                four_tables(tab, 0, ss[(i + 3) % 4], rf2(0, 3)) ^
                four_tables(tab, 1, ss[(i + 3) % 4], rf2(1, 3)) ^
        ...
        private static void kdl4(uint[] k, int i, uint[] ss, uint[][] tab)
        {
            ss[4] =
                four_tables(tab, 0, ss[(i + 3) % 4], rf2(0, 3)) ^
                four_tables(tab, 1, ss[(i + 3) % 4], rf2(1, 3)) ^
        ...
        private static uint upr(uint x, uint n)
        {
        ...
        /////////////////////////////////// encrypt ////////////////////////////////////////
        private static byte[] EncryptBuf(byte[] data, aes_ctx ctx)
        {
            ...
            if (BLOCK_LEN > rlen) // file length of less than one block
            {
        ...
        private static void EncryptBlk(byte[] in_blk, byte[] out_blk, aes_ctx ctx)
        {
            uint[] b0 = new uint[4];
            uint[] b1 = new uint[4];
        ...
        private static void fillrand(byte[] buf, int len)
        {
        ...
        private static uint fwd_var(uint[] x, int r, int c, int nc)
        {
            return
               r == 0 ?
                (c == 0 ? x[0]
        ...
        private static void fwd_rnd(uint[] y, uint[] x, uint[] k, int index, int c, int nc, uint[][] tab)
        {
            y[c] = k[c + index] ^
        ...
        private static void state_in(uint[] y, byte[] x, uint[] k, int index)
        {
            for (int i = 0; i < 4; i++)
            {
        ...
        private static aes_ctx EncryptKey(string strKey)
        {
            aes_ctx ctx = null;
        ...
        private static void ke4(uint[] k, int i, uint[] ss)
        {
            k[4 * i + 4] = ss[0] ^=
        ...
        private static uint four_tables(uint[][] tab, uint m, uint vf, uint rf)
        {
            return tab[m][bval(vf, rf)];
        }

        private static byte bval(uint x, uint n)
        {
            return (byte)(x >> (int)(8 * n));
        }

        private static uint rf2(uint r, uint c)
        {
            return (r - c) & 3;
        }
        /////////////////////////////////// encrypt ////////////////////////////////////////

        private static void memset(byte[] buff, int index, byte val)
        {
        ...
        private const int BPOLY = 0x1b;
        private const uint m1 = 0x80808080;
        private const int m2 = 0x7f7f7f7f;
        private const int BLOCK_LEN = 16;
        private const int BLOCK_SIZE = 16;
        private const int KS_LENGTH = 4 * BLOCK_SIZE;
        private const int FIX_KEY_LEN = 16;
        public const string KEY = "f83od0gmnr4y";

        private static uint[] rcon_tab = new uint[29] {
        ...
        private static uint[][] fl_tab = new uint[4][]{
			new uint[256] {
        ...
        private static uint[][] ft_tab = new uint[4][]{
			new uint[256] {
        ...
        private static uint[][] il_tab = new uint[4][]{
			new uint[256] {
        ...
        private static uint[][] it_tab = new uint[4][]{
			new uint[256]{
